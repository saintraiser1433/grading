...
interface ClassGradingComponentsProps {
...
export function ClassGradingComponents({ gradeTypes, globalCriteria }: ClassGradingComponentsProps) {
  const [activeTab, setActiveTab] = useState(gradeTypes[0]?.id || "")
  ...
  // Group criteria by grade type
  const criteriaByGradeType = gradeTypes.map(gradeType => ({
    gradeType,
    criteria: globalCriteria.filter(c => c.gradeTypeId === gradeType.id)
  }))
  ...
  return (
    <div className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        {criteriaByGradeType.map(({ gradeType, criteria }) => (
          <TabsContent key={gradeType.id} value={gradeType.id} className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <BookOpen className="h-5 w-5" />
                  {gradeType.name} Components
                </CardTitle>
                <CardDescription>
                  {gradeType.description || `Manage grading components for ${gradeType.name.toLowerCase()}`}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {criteria.length === 0 ? (
                    <div className="text-center py-8 border rounded-lg bg-muted/50">
                      <p className="text-muted-foreground">No criteria available for {gradeType.name}</p>
                      <p className="text-sm text-muted-foreground mt-2">
                        Contact administrator to set up grading criteria for this grade type.
                      </p>
                    </div>
                  ) : (
                    criteria.map((criterion) => (
                      <div key={criterion.id} className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <Badge variant="outline">
                              {criterion.componentDefinitions.length} components
                            </Badge>
                            <Dialog open={open && componentForm.criteriaId === criterion.id} onOpenChange={setOpen}>
                              <DialogTrigger asChild>
                                <Button 
                                  size="sm"
                                  onClick={() => handleOpenDialog(undefined, criterion.id)}
                                >
                        ...
                        {criterion.componentDefinitions.length === 0 ? (
                          <p className="text-sm text-muted-foreground ml-4">No components added yet</p>
                        ) : (
                          <div className="grid gap-2 ml-4">
                            {criterion.componentDefinitions.map((component) => (
                              <div
                                key={component.id}
